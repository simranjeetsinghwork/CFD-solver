!g95 -O2 -o vkt -Wall -Wextra -fbounds-check vkt_airfoil_v2_cylinder.f90
!********************************************************************************
...
!Airfoil input parameters (See Section 7.10.4 of 'I do like CFD, VOL.1')
 ...
 real(dp), parameter :: epsilon = 0.1_dp               !Thickness
...
!Grid input parameters
  integer :: nx =  320          !# of nodes around the airfoil (odd number)
  integer :: ny =  160          !# of nodes in the radial direction
! real(dp), parameter :: distance = 40.0_dp ! Distance to the outer boundary
 real(dp), parameter :: distance = 500.0_dp ! Distance to the outer boundary
! real(dp), parameter :: stretch_factor = 6.8_dp !Stretching factor in r-direction (>1.0)
 real(dp), parameter :: stretch_factor = 10.0_dp !Stretching factor in r-direction (>1.0)
...
!Local variables
 ...
 if (igrid==1) then
  nx = 40
  ny = 20
  grid_file_tria = "vkt_airfoil_040x020_tria.grid"
   tec_file_tria = "vkt_airfoil_040x020_tria_tec.dat"
   vtk_file_tria = "vkt_airfoil_040x020_tria.vtk"
  grid_file_quad = "vkt_airfoil_040x020_quad.grid"
   tec_file_quad = "vkt_airfoil_040x020_quad_tec.dat"
   vtk_file_quad = "vkt_airfoil_040x020_quad.vtk"
 elseif (igrid==2) then
  nx = 80
  ny = 40
  grid_file_tria = "vkt_airfoil_080x040_tria.grid"
   tec_file_tria = "vkt_airfoil_080x040_tria_tec.dat"
   vtk_file_tria = "vkt_airfoil_080x040_tria.vtk"
  grid_file_quad = "vkt_airfoil_080x040_quad.grid"
   tec_file_quad = "vkt_airfoil_080x040_quad_tec.dat"
   vtk_file_quad = "vkt_airfoil_080x040_quad.vtk"
 elseif (igrid==3) then
  nx = 160
  ny =  80
  grid_file_tria = "vkt_airfoil_160x080_tria.grid"
   tec_file_tria = "vkt_airfoil_160x080_tria_tec.dat"
   vtk_file_tria = "vkt_airfoil_160x080_tria.vtk"
  grid_file_quad = "vkt_airfoil_160x080_quad.grid"
   tec_file_quad = "vkt_airfoil_160x080_quad_tec.dat"
   vtk_file_quad = "vkt_airfoil_160x080_quad.vtk"
 endif
...
! 1. Grid generation for a specified Karman-Trefftz airfoil.
 ...
 call generate_grids_kt_airfoil(ell,alpha,epsilon,kappa,tau, nx,ny,distance,&
                      ...
                      grid_file_tria,tec_file_tria,grid_file_quad,tec_file_quad)
...
!Input
 ...
 character(80), intent(in) :: grid_file_tria, tec_file_tria
 character(80), intent(in) :: grid_file_quad, tec_file_quad
...
!    r = r*(1.0d0 + (rn-0.5d0) * 0.006d0)
  ...
  interior_j : do j = 1, nx
   ...
   theta = real(j-1,dp)*(two*pi)/real(nx,dp)
    call random_number(rn)
!   if (k<10)   r = r_base + sign(rn-0.5d0,1.0d0)*dr * 0.37d0
 
 if (j>1) then
  dtheta = theta - theta_pre! real(j-2,dp)*(two*pi)/real(nx,dp)
 else
  dtheta = real(1,dp)*(two*pi)/real(nx,dp)
 endif
   theta = theta + 0.0d0 !sign(rn-0.5d0,1.0d0)*dtheta * 1.7d0
   theta_pre = theta
...
!  dZdzeta = dZ/dzeta: the derivative of the KT transformation
  ...
  do k = 1, ny-1
    do j = 1, nx-1
    call random_number(rn)
    if (j>1 .and. j<nx-1 .and. k>1 .and. k<ny-1) then
      ds = sqrt((x(j,k)-x(j-1,k))**2 + (y(j,k)-y(j-1,k))**2)
!      x(j,k) = x(j,k) + (rn-half)*ds*0.35d0
!      y(j,k) = y(j,k) - (rn-half)*ds*0.35d0
    endif
    end do
  end do
...
! vanishing elements at the trailing edge.
  do k = 1, ny-1
    ...
    rn2 = rn-0.5d0
     ...
     if (j <= nx/2 ) then
!     if (rn2 > 0.0d0) then
      i1 = nx*(k-1)+j  ; i2 = nx*k+j; i3 = nx*(k-1)+j+1
      i4 = nx*(k-1)+j+1; i5 = nx*k+j; i6 = nx*k+j+1

      write(11,*) i1,i2,i3
      write(11,*) i4,i5,i6

        x1 = xx(i1); x2 = xx(i2); x3 = xx(i3);
        y1 = yy(i1); y2 = yy(i2); y3 = yy(i3);
        vol = 0.5d0*( x1*(y2-y3) + x2*(y3-y1) + x3*(y1-y2) )
        if (vol<0.0d0) then
         write(*,*) "Negative volume : ", vol
          negative_volume = .true.
        endif
        x1 = xx(i4); x2 = xx(i5); x3 = xx(i6);
        y1 = yy(i4); y2 = yy(i5); y3 = yy(i6);
        vol = 0.5d0*( x1*(y2-y3) + x2*(y3-y1) + x3*(y1-y2) )
        if (vol<0.0d0) then
         write(*,*) "Negative volume : ", vol
          negative_volume = .true.
      ...
      i1 = nx*(k-1)+j+1; i2 = nx*(k-1)+j; i3 = nx*k+(j+1)
      i4 = nx*(k-1)+j  ; i5 =     nx*k+j; i6 = nx*k+j+1
      ...
      j = nx
     if (j <= nx/2 ) then
      i1 = nx*(k-1)+j; i2 = nx*k+j; i3 = nx*(k-1)+1
      i4 = nx*(k-1)+1; i5 = nx*k+j; i6 =   nx*k+1
      ...
      i1 = nx*(k-1)+j; i2 = nx*k+1; i3 = nx*(k-1)+1
      i4 = nx*(k-1)+j; i5 = nx*k+j; i6 =     nx*k+1
