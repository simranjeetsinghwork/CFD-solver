...
!
! This is module_implicit_solver.
!
!
...
!********************************************************************************
 ...
 !This module contains the following subroutine:
  ...
  public :: implicit_steady_solver   ! solve a steady problem with local time step.
  public :: implicit_unsteady_solver ! solve an unsteady problem.
...
!
!*******************************************************************************
 subroutine implicit_steady_solver

  use module_common_data    , only : p2, zero, one, half, filename_tec_hist
 ...
 !Custom data type for Jacobian
  ...
  type(cc_data_type), dimension(:), pointer :: jac

  integer                :: i_iteration, i, stat, i_temp
  real(p2)               :: temp
  real(p2), dimension(4) :: res_norm
  real(p2), dimension(4) :: res_norm_initial
  real(p2), dimension(ncells,4) :: du

  allocate(jac(ncells))
  do i = 1, ncells
   allocate( jac(i)%diag(4,4) )
   allocate( jac(i)%off( cell(i)%nnghbrs, 4,4) )
   allocate( jac(i)%inverse_diag(4,4) )
  end do
 ...
 !-----------------------------------------------------------------------------
  ...
  !------------------------------------------------------
  ! Update the solution by an implicit solver

    call compute_jacobian(jac)
    call linear_relaxation(jac,res, du)
 ...
 end subroutine implicit_steady_solver
!*******************************************************************************
...
!
!*******************************************************************************
 subroutine implicit_unsteady_solver
 ...
 end subroutine implicit_unsteady_solver
!*******************************************************************************
